...
def EncryptCaesarCipher(plainText, key):
    cipherText = ''
    for plainTextLetter in plainText: # this is plainText @#$%^^&**(*(
            asci = ord(plainTextLetter) # convert to unicode code: a = 97
            cipherText += chr(asci+key) # convert unicode code to character 100 = c
    return cipherText
...
def affineCipherDecryption(cypherText, k1, k2):
    plainText = ''
    for cypherTextLetter in cypherText:
        newIndex = (modinv(k1)) * (alphabet.find(cypherTextLetter) - k2) % len(alphabet)
